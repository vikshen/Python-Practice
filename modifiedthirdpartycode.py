...
def check(interest, frozen):
    for intEx in interest:
        try:
            intEx=dicti[intEx]+intEx
        except KeyError:
            intEx="No_SEC"+intEx
        if((intEx+'_DOWN' in frozen) or (intEx+'_UP' in frozen)):
            #print('checked')
            return True
...
def calc_confidence(freqSet, H, support_data, rules, interest, part='antecedent', min_confidence=0.6):
    ...
    for conseq in H:
        ...
        if conf >= min_confidence:
            if(part=='antecedent'):
                if(check(interest, freqSet - conseq)):
                    print (freqSet - conseq, '--->', conseq, 'conf:', conf)
                    rules.append((freqSet - conseq, conseq, conf))
                    pruned_H.append(conseq)
            else:
                if(check(interest, conseq)):
                    print (freqSet - conseq, '--->', conseq, 'conf:', conf)
                    rules.append((freqSet - conseq, conseq, conf))
                    pruned_H.append(conseq)
    return pruned_H
...
dicti = {}
wb = xlrd.open_workbook('/Users/vikasshenoy/Desktop/Financial Data Mining/assignment_07(1)/stocks/SP500Companies.xlsx','r')
sh = wb.sheet_by_index(0)   
for i in range(503):
    cell_value_class = sh.cell(i,0).value
    cell_value_id = sh.cell(i,2).value
    dicti[cell_value_class] = cell_value_id